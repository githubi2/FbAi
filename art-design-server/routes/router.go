	...
	v1 := r.Group("/api/v1")
	{
		...
		authorized.Use(middleware.TenantContext())
		{
			...
			fb := authorized.Group("/fb")
			{
				...
				fb.GET("/pixels", handlers.DefaultFbHandler.PixelList)
				fb.POST("/pixels", handlers.DefaultFbHandler.CreatePixel)
				fb.POST("/pixels/refresh-all", handlers.DefaultFbHandler.RefreshAllPixels)
				fb.PUT("/pixels/:id/remark", handlers.DefaultFbHandler.UpdatePixelRemark)
